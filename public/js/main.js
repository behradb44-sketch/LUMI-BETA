...
let ws=null,myId=null,room=null,world={w:3200,h:2100},players=new Map(),keys={},camera={x:0,y:0},last=performance.now(),particles=[];let connectionToken=0;
...
function connect(code,create=false){const token=++connectionToken;if(ws){try{ws.onmessage=null;ws.onclose=null;ws.close()}catch{}}players.clear();myId=null;room=null;ws=new WebSocket(wsUrl(code,create));toast('Connecting…');ws.onopen=()=>{if(token!==connectionToken)return;toast('Connected')};ws.onmessage=e=>{if(token!==connectionToken)return;try{handle(JSON.parse(e.data))}catch{}};ws.onclose=()=>{if(token!==connectionToken)return;if(!game.classList.contains('hidden'))toast('Connection lost — refresh to reconnect')};ws.onerror=()=>{if(token===connectionToken)toast('Connection error')}}
function handle(m){
 if(m.type==='error'){toast(m.message);return}
 if(m.type==='welcome'){if(m.protocol&&m.protocol!==3){toast('Protocol mismatch — refresh the page');return}myId=m.id;room=m.room;world=m.world;players.clear();for(const p of room.players)players.set(p.id,{...p,tx:p.x,ty:p.y});showGame();updateUI();return}
 if(m.type==='room-state'){if(!room||!m.room||m.room.code!==room.code)return;room=m.room;const next=new Map();for(const p of room.players){const old=players.get(p.id);next.set(p.id,old?Object.assign(old,p):{...p,tx:p.x,ty:p.y})}players=next;updateUI();return}
 if(m.type==='player-joined'){if(!room)return;players.set(m.player.id,{...m.player,tx:m.player.x,ty:m.player.y});toast(`${m.player.name} joined the world`);updateUI();return}
 if(m.type==='player-left'){players.delete(m.id);updateUI();return}
 if(m.type==='snapshot'){if(!room)return;const next=new Map();for(const p of m.players){const old=players.get(p.id);if(old){old.tx=p.x;old.ty=p.y;Object.assign(old,p);next.set(p.id,old)}else next.set(p.id,{...p,tx:p.x,ty:p.y})}players=next;return}
