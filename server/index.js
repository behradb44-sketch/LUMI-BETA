...
class Room{
 constructor(code,name,permanent=false){
  this.code=code;this.name=name;this.permanent=permanent;this.private=!permanent && false;this.players=new Map();this.started=false;this.phase=0;this.shards=new Set();this.objective={need:3,got:0,label:'Restore the first Memory Beacon'};this.created=Date.now();this.startedAt=0;this.hostId=null;this.lastActivity=Date.now();
 ...
 state(){return {protocol:PROTOCOL,code:this.code,name:this.name,permanent:this.permanent,private:!!this.private,started:this.started,phase:this.phase,objective:this.objective,players:this.snapshot(),count:this.players.size,max:MAX_PLAYERS,hostId:this.hostId,beacons:this.beacons}}
...
function publicList(){return [...rooms.values()].filter(r=>!r.private).map(r=>({code:r.code,name:r.name,count:r.players.size,max:MAX_PLAYERS,permanent:r.permanent,started:r.started,phase:r.phase}));}
...
wss.on('connection',(ws,req)=>{
 ...
 if(roomCode==='CREATE'){room=new Room(code(),cleanName(u.searchParams.get('name')),false);room.private=u.searchParams.get('private')==='1';rooms.set(room.code,room)}
 ...
 ws._lumiRoom=room.code;ws._lumiPlayer=id;ws.send(JSON.stringify({type:'welcome',protocol:PROTOCOL,id,room:room.state(),world}));room.broadcast({type:'player-joined',player:{id:p.id,name:p.name,color:p.color,x:p.x,y:p.y}},id);room.broadcast({type:'room-state',room:room.state()});
 ...
 ws.on('close',()=>{if(ws._lumiClosed)return;ws._lumiClosed=true;if(room.players.has(id)){room.remove(id);room.broadcast({type:'player-left',id});room.broadcast({type:'room-state',room:room.state()})}});
});
